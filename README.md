...
Publish (with versioning and release) is taken care by [`semantic-release`](https://github.com/semantic-release/semantic-release). Just follow commit guidelines, commit using `npm run commit` each time and push, and semantic-release will handle the rest.
...
| `release.yml` | ✔️ All Branches | ✔️ Lint<br>✔️ Build<br>✔️ Test<br>✔️ Versioning based on the [Semantic Versioning](http://semver.org/) specification.<br>✔️ Publish library on specific channel<br>✔️ Make release tag on GitHub<br>✔️ Adds `released@channel` label and friendly comments on issues<br><br>*Checkout [Release workflow](https://semantic-release.gitbook.io/semantic-release/recipes/recipes#release-workflow) for better understanding on version and release management for different branches* |
