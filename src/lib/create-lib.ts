...
function updatePackageJSON(options: Schema, libPath: string, tree: Tree, scopeWithName: string): Tree {
  ...
  return setJSON(tree, packageJSONPath, { ...pkg, ...packageJSONExtensions(options, scopeWithName) });
}
